...
const resolveSegmentAngle = (segmentCount: number): number =>
    FULL_ROTATION / segmentCount;

const pickRandomIntInclusive = (
    min: number,
    max: number,
    rng: () => number,
): number => {
...
export const planSpin = <T>(
    ...
    const minRotations =
        options.minRotations ?? config.minRotations ?? DEFAULT_MIN_ROTATIONS;
    const maxRotations =
        options.maxRotations ?? config.maxRotations ?? DEFAULT_MAX_ROTATIONS;
    const rotations = pickRandomIntInclusive(minRotations, maxRotations, rng);
    const durationMs =
        options.durationMs ?? config.durationMs ?? DEFAULT_DURATION_MS;
    const rotationDirection =
        config.rotationDirection ?? DEFAULT_ROTATION_DIRECTION;
    ...
    if (hasTargetAngle) {
        ...
        winningIndex = indexFromAlignmentAngle(
            baseAngle,
            pointerAngle,
            segmentAngle,
            segments.length,
    ...
    if (rotationDirection === 1) {
        ...
        delta = -(
            (currentNormalized - baseAngle + FULL_ROTATION) %
            FULL_ROTATION
        );
        delta -= rotations * FULL_ROTATION;
    }
